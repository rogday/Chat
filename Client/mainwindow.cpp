...
{
    client.asyncSend(Client::Event::Auth, ui->login->toPlainText().toStdString() + ':' + ui->password->toPlainText().toStdString());
}

void MainWindow::on_submit_2_clicked()
{
    client.asyncSend(Client::Event::Room, ui->roomname->toPlainText().toStdString());
}
